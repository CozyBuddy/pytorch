...
print('Vocab Transform :' , vocab_transform)
...
class Seq2SeqTransformer(nn.Module):
    ...
    def forward(self,src,trg,src_mask , tgt_mask , src_padding_mask ,tgt_padding_mask, memory_key_padding_mask):
        ...
        outs = self.transformer(
            src=src_emb,
            tgt=tgt_emb, src_mask=src_mask ,tgt_mask=tgt_mask , memory_mask=None,
...
model = Seq2SeqTransformer(
    num_encoder_layers=3, num_decoder_layers=3 , emb_size=512 , max_len=512 , nhead=8 , src_vocab_size = len(vocab_transform[SRC_LANG]) , tgt_vocab_size= len(vocab_transform[TGT_LANG]) , dim_feedforward=512
...
dataloader = DataLoader(
    data_iter, batch_size=BATCH_SIZE , collate_fn=collator 
)
...
def generate_square_subsequent_mask(s):
    mask = (
        torch.triu(torch.ones((s,s) , device=DEVICE)) == 1
    ).transpose(0,1)
...
def run(model , optimizer, criterion, split):
    ...
    for source_batch , target_batch in dataloader:
        ...
        logits = model(
            src=source_batch, trg=target_input, src_mask=src_mask , tgt_mask=tgt_mask, src_padding_mask= src_padding_mask , tgt_padding_mask = tgt_padding_mask, memory_key_padding_mask = src_padding_mask
        )
